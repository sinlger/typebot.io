...
function Cell({ value }: { value: string | boolean }) {
  if (value === true)
    return <Check className="w-4 h-4 text-brand-600 mx-auto" />;
  if (value === false)
        ...
        <table className="w-full border-collapse min-w-180">
          <thead>
            <tr className="border-b border-slate-200">
              <th className="text-left text-sm font-semibold text-slate-500 py-4 pr-4 w-1/3">
                功能
              </th>
              {pricingPlans.map((plan) => (
                <th
                  key={plan.name}
                  className="text-center text-sm font-bold text-slate-900 py-4 px-3"
                >
                  {plan.name}
                </th>
              ))}
              ...
              <Fragment key={group.title}>
                ...
                {group.rows.map((row) => (
                  <tr key={row.label} className="border-b border-slate-100">
                    <td className="text-sm text-slate-600 py-3.5 pr-4">
                      {row.label}
                    </td>
                    {row.values.map((v, i) => (
